...
- **[Agent Artifacts](./docs/agent_artifacts.md)**: Files generated by agents via `write_file`, streamed in real-time.
...
- **Key Commands**:
  ...
  - _Note: Always use `--run` (e.g., `yarn test:nuxt path/to/test --run`) to execute once without watch mode._
